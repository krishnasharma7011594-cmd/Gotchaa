...
void main() async {
  ...
  final files = l10nDir.listSync().whereType<File>().where((f) => f.path.endsWith('.json')).toList();
  ...
  buffer.writeln("import 'package:flutter_localizations/flutter_localizations.dart';");
  buffer.writeln('');
  ...
  buffer.writeln('');
  buffer.writeln('  const LanguageMeta(this.code, this.nameNative, this.nameEn, this.flag);');
  buffer.writeln('}');
  buffer.writeln('');
  ...
  buffer.writeln('  static String tr(String key, String locale, {List<String>? args, Map<String, String>? namedArgs}) {');
  buffer.writeln("    String value = data[locale]?[key] ?? data['en']?[key] ?? key;");
  ...
  for (final lang in translations.keys) {
    ...
    for (final key in map.keys) {
      ...
      final val = map[key]!.replaceAll(r'\', r'\\').replaceAll("'", r"\'").replaceAll(r'$', r'\$').replaceAll('\n', r'\n');
  ...
  buffer.writeln('  String tr(String key, {List<String>? args, Map<String, String>? namedArgs}) {');
  buffer.writeln('    final locale = Localizations.localeOf(this).languageCode;');
  buffer.writeln('    return TrData.tr(key, locale, args: args, namedArgs: namedArgs);');
  buffer.writeln('  }');
  buffer.writeln('}');
  
  final outFile = File('lib/core/l10n/app_localizations_x.dart');
  outFile.writeAsStringSync(buffer.toString());
  print(r'Generated app_localizations_x.dart with ${translations.length} languages!');
}
