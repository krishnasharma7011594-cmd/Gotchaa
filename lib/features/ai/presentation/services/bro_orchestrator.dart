...
class BroOrchestrator {
  ...
  Future<BroResponse> _processVoiceInput(File audioFile) async {
    ...
    try {
      ...
      // 2. Play TTS Response if available (FastAPI returning audio_url)
      ...
      // 3. Fallback to Gemini if FastAPI is down or fails
      return await _fallbackToGemini("Incoming voice transcript placeholder (STT failed on backend)", startTime);
    } finally {
  ...
  Future<BroResponse> _fallbackToGemini(String query, DateTime startTime) async {
    if (_geminiFallback == null) return BroResponse.failed("Both FastAPI and Gemini fallback are unavailable.");
    ...
    try {
      ...
      return BroResponse.failed("Primary backend offline and Gemini fallback failed: $e");
    }
  }
  ...
  BroResponse _parseFastApiResponse(dynamic responseData, double executionTime) {
    ...
    return BroResponse(
      ...
      text: responseData['text'] ?? "Done, boss!",
  ...
  Future<bool> _authenticateUser() async {
    try {
      ...
      if (!canCheck) return true; // Fallback for devices without biometrics

      return await _localAuth.authenticate(
        localizedReason: 'Verify identity for secure BRO action',
