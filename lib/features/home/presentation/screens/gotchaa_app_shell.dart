...
class _GotchaaAppShellState extends ConsumerState<GotchaaAppShell> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (ref.read(shellPageIndexProvider) == 0 &&
          ref.read(shellPageControllerProvider).hasClients) {
  ...
  Future<void> _maybeShowConsentModal() async {
    ...
    _consentModalShown =
        true; // Synchronously guard to prevent concurrent race-condition dialogs
  ...
  Widget build(BuildContext context) {
    ...
    return OfflineBanner(
      child: Scaffold(
        ...
        bottomNavigationBar:
            _buildBottomNav(currentPage, isUnverified, isLimited),
      ),
    );
  }
              ...
              TextButton(
                onPressed: () {
                  ...
                  _showLockedFeatureNotice(
                    title: 'Full Access',
                    message:
                        'Enter an invite code to unlock premium exclusive mini-apps and features.',
                  );
                },
  ...
  Widget _buildBottomNav(int currentPage, bool isUnverified, bool isLimited) {
    ...
    final bgColor = isDark
        ? const Color(0xFF161921).withOpacity(0.9)
  ...
  Widget _buildCreateButton(bool isDark, bool isUnverified, bool isLimited) =>
      GestureDetector(
        onTap: () async {
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child:
                Text('Maybe later', style: TextStyle(color: Colors.grey[500])),
          ),
          ElevatedButton(
            onPressed: () async {
