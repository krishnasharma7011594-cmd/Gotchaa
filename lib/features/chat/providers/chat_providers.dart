  ...
  return ref
    ...
    final chats = snapshot.docs
      ...
      if (otherParticipant.isNotEmpty &&
          blockedUids.contains(otherParticipant)) {
        return false;
      }
      return true;
    }).toList();
...
String? _validateMessageDoc(Map<String, dynamic> raw) {
  final sender =
      (raw['senderId'] ?? raw['senderID'] ?? raw['sender'] ?? raw['from'])
              ?.toString()
              .trim() ??
          '';
  if (sender.isEmpty) return 'missing senderId';

  final type = (raw['type']?.toString().trim().isNotEmpty ?? false)
  ...
  final hasText =
      (raw['text'] ?? raw['message'] ?? raw['body'] ?? raw['content'])
              ?.toString()
              .trim()
              .isNotEmpty ??
          false;
...
final typingProvider = StreamProvider.family<Map<String, bool>, String>(
    (ref, chatId) => ref
            .watch(firestoreProvider)
            .collection('chats')
            .doc(chatId)
...
final userPresenceProvider = StreamProvider.family<UserProfile?, String>(
    (ref, userId) => ref
            .watch(firestoreProvider)
            .collection('users')
            .doc(userId)
