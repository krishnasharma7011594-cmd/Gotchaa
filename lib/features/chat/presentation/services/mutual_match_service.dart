...
class MutualMatchService {
  ...
  Stream<SessionMatchState> watchSession(String sessionId, String myId) =>
      _firestore
          .collection('randomSessions')
          .doc(sessionId)
          .snapshots()
          .map((doc) {
        if (!doc.exists) {
          return SessionMatchState(
              meWantsToKeep: false, otherWantsToKeep: false, isMutual: false);
        }
        final data = doc.data()!;
        final isA = data['userAId'] == myId;

        final meWants = isA
            ? (data['userAWantsToKeep'] ?? false)
            : (data['userBWantsToKeep'] ?? false);
        final otherWants = isA
            ? (data['userBWantsToKeep'] ?? false)
            : (data['userAWantsToKeep'] ?? false);
        ...
        return SessionMatchState(
          ...
          isMutual: mutual,
          requestedAt: isA
              ? (data['userARequestedAt'] as Timestamp?)?.toDate()
              : (data['userBRequestedAt'] as Timestamp?)?.toDate(),
          status: data['status'],
        );
      });
    ...
    await _firestore.runTransaction((transaction) async {
      ...
      if (isA) {
        transaction.update(docRef, {
          ...
          'status': data['userBWantsToKeep'] == true
              ? 'mutualMatch'
              : 'keepRequested',
          if (data['userBWantsToKeep'] == true)
        ...
        transaction.update(docRef, {
          ...
          'status': data['userAWantsToKeep'] == true
              ? 'mutualMatch'
              : 'keepRequested',
          if (data['userAWantsToKeep'] == true)
            'mutualMatchAt': FieldValue.serverTimestamp(),
        });
      }
    });
...
class KeepTalkingButton extends StatefulWidget {
  const KeepTalkingButton(
      {required this.sessionId, required this.onRequested, super.key});
...
class _KeepTalkingButtonState extends State<KeepTalkingButton>
...
class KeepTalkingPromptSheet extends StatelessWidget {
  const KeepTalkingPromptSheet(
      {required this.onAccept, required this.onDecline, super.key});
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(
          ...
          begin: 1, end: 0, curve: Curves.easeOutBack, duration: 500.ms);
}

class MutualMatchCelebrationScreen extends StatefulWidget {
  const MutualMatchCelebrationScreen(
      {required this.onComplete, super.key, this.gameName});
  final VoidCallback onComplete;
  final String? gameName;

  @override
  State<MutualMatchCelebrationScreen> createState() =>
      _MutualMatchCelebrationScreenState();
}

class _MutualMatchCelebrationScreenState
    extends State<MutualMatchCelebrationScreen> {
                ...
                AnimatedOpacity(
                  ...
                  child: CachedNetworkImage(
                    ...
                    placeholder: (context, url) => Container(
                      ...
                      child:
                          const BlurHash(hash: 'L5H2EC=pPdpWXVJs00QQV_9H00XY'),
                    ),
          ...
          Column(
            ...
            children: [
              Text('Anonymous 👤',
                  style: GoogleFonts.outfit(
                      fontWeight: FontWeight.bold, color: Colors.white)),
              Text('Matched via Random Chat',
                  style:
                      GoogleFonts.outfit(fontSize: 10, color: Colors.white38)),
            ],
          ),
        ],
      );
...
class ConnectionMemoryWidget extends StatelessWidget {
  const ConnectionMemoryWidget(
      {required this.metVia, required this.metAt, super.key});
  final String metVia;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(
            metVia.contains('video')
                ? Icons.videocam
                : (metVia.contains('game')
                ...
                child: ElevatedButton(
                  onPressed: () {
                    if (userId != null) {
                      ...
                      Navigator.pop(
                          context); // Feedback handled by session listener elsewhere
                    }
                  },
                  style: ElevatedButton.styleFrom(
                    backgroundColor: AppColors.electricBlue,
