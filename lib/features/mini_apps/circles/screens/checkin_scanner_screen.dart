...
class _CheckInScannerScreenState extends State<CheckInScannerScreen> {
  ...
  Future<void> _checkProximity() async {
    ...
    final inRange = await CirclesCheckInService.instance.isWithinProximity(widget.circle);
    setState(() {
  ...
  Future<void> _triggerSimulatedQrScan() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('🎉 QR Code Check-in Verified! You gained +10 Karma points!')),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _triggerProximityCheckIn() async {
    try {
      ...
      _confettiController.play();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('🎉 Proximity GPS Check-in Verified! You gained +10 Karma points!')),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Verification Failed: $e')),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: context.bg,
        title: Text('Circle Check-In', style: GoogleFonts.outfit(color: Colors.white, fontWeight: FontWeight.bold)),
      ),
      body: SafeArea(
        child: Stack(
                      ...
                      child: Center(
                        child: Container(
                          ...
                          child: const Stack(
                            alignment: Alignment.center,
                            children: [
                              Icon(Icons.qr_code_scanner_rounded, color: AppColors.primaryGlow, size: 64),
                              Positioned(
                                bottom: 16,
                                child: Text(
                    ...
                    GlassmorphicCard(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          children: [
                            Icon(
                              _isWithinRange ? Icons.gps_fixed_rounded : Icons.gps_not_fixed_rounded,
                              color: _isWithinRange ? Colors.green : AppColors.karmaOrange,
                            ),
