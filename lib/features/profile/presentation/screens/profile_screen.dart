...
class _ProfileView extends ConsumerWidget {
      ...
      actions: [
        IconButton(
          onPressed: () {
            ...
            Share.share(context.tr('profile_share_msg', args: [profile.inviteCode]));
  ...
  Widget _buildProfileHeader(BuildContext context, WidgetRef ref) => Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            profile.bio.isNotEmpty ? profile.bio : context.tr('profile_bio_empty'),
            style: GoogleFonts.outfit(
      ...
      child: Row(
        ...
        children: [
          ...
          InkWell(
            ...
            child: _buildStat(
                context, profile.followersCount.toString(), context.tr('profile_followers')),
          ),
          InkWell(
            onTap: () => Navigator.push(
              context,
            ...
            child: _buildStat(
                context, profile.followingCount.toString(), context.tr('profile_following')),
          ),
          _buildStat(
            context,
            (isUnverified && !profile.isLimitedUser) ? '🔒' : profile.karma.toString(),
  ...
  Widget _buildStat(BuildContext context, String value, String label, {bool isBlurred = false}) => Column(children: [
      if (isBlurred)
        ImageFiltered(
  ...
  Widget _buildKarmaBadge(BuildContext context, WidgetRef ref) => GestureDetector(
      onTap: () {
        ...
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (_) => const KarmaDashboardScreen()));
      },
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          if (isUnverified && !profile.isLimitedUser)
            ImageFiltered(
              ...
              child: Text(context.tr('profile_karma_score', args: [profile.karma.toString()]),
                  style: GoogleFonts.outfit(
                      color: Colors.white,
                      fontWeight: FontWeight.bold,
                      fontSize: 16)),
            ...
            Text(context.tr('profile_karma_score', args: [profile.karma.toString()]),
                style: GoogleFonts.outfit(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
                    fontSize: 16)),
  ...
  Widget _buildInviteCodeCard(BuildContext context) => Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.people_alt_rounded, color: AppColors.electricBlue, size: 20),
              const SizedBox(width: 8),
              Text(
          ...
          if (!profile.isVerified)
            ...
            Row(
              children: [
                _buildUsageStat(context, '${profile.remainingInvites}', context.tr('profile_invite_left')),
                const SizedBox(width: 12),
                _buildUsageStat(context, '${profile.totalInvites}', context.tr('profile_invite_total')),
                const Spacer(),
                Container(
                  padding:
                ...
                IconButton(
                  onPressed: () async {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text(context.tr('profile_invite_copied'))),
                    );
                  },
                ...
                IconButton(
                  onPressed: () {
                    Share.share(context.tr('profile_invite_msg_alt', args: [profile.inviteCode]));
                    AnalyticsService.logInviteSent();
                  },
  ...
  Widget _buildUsageStat(BuildContext context, String value, String label) => Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(value, style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold, color: context.textPrimary)),
        Text(label, style: GoogleFonts.outfit(fontSize: 10, color: context.textHint)),
      ],
    );
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: Text(context.tr('btn_maybe_later'), style: TextStyle(color: context.textHint)),
          ),
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
  ...
  Widget _buildVybzGrid(BuildContext context, WidgetRef ref, String uid) {
    ...
    return vybzAsync.when(
      data: (list) {
        if (list.isEmpty) return _buildEmptyTab(context, context.tr('profile_empty_vybz'));
        return GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Stack(
              fit: StackFit.expand,
              children: [
                if (vybz.videoUrl.isNotEmpty) CachedNetworkImage(
                        ...
                        errorWidget: (_, __, ___) =>
                            Container(color: context.shimmerBase),
                      ) else Container(color: context.shimmerBase),
                const Center(
      ...
      error: (e, _) => _buildEmptyTab(context, context.tr('profile_error_vybz')),
    );
  }
      ...
      error: (e, _) => Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.error_outline_rounded,
              size: 40, color: context.iconMuted),
          const SizedBox(height: 12),
...
class _SliverTabBarDelegate extends SliverPersistentHeaderDelegate {
  ...
  @override
  Widget build(BuildContext context, double shrinkOffset, bool overlapsContent) => Container(color: _context.surface, child: _tabBar);
