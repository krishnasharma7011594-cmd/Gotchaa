...
final _otherProfileProvider =
    StreamProvider.family<UserProfile?, String>((ref, uid) => ref.watch(firestoreRepositoryProvider).getUserProfileStream(uid));
...
class UserProfileScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return profileAsync.when(
      ...
      error: (e, _) => Scaffold(
        appBar: AppBar(),
        body: Center(child: Text(context.tr('error_prefix', args: [e.toString()]))),
      ),
    );
  }
}
...
class _OtherProfileViewState extends ConsumerState<_OtherProfileView> {
  ...
  void didUpdateWidget(covariant _OtherProfileView oldWidget) {
    ...
    if (!_isFollowingInitialized && widget.profile.followersCount != oldWidget.profile.followersCount) {
      _followersCountLocal = widget.profile.followersCount;
    }
  }
  ...
  void _toggleFollow() {
    ...
    if (willFollow) {
      // 1. Perform Follow
      social.followUser(myUid: widget.myUid, targetUid: widget.profile.uid).catchError((e) {
         if (mounted) {
           setState(() {
              _isFollowingLocal = !willFollow;
              _followersCountLocal += willFollow ? -1 : 1;
      ...
      social.unfollowUser(myUid: widget.myUid, targetUid: widget.profile.uid).catchError((e) {
         if (mounted) {
           setState(() {
              _isFollowingLocal = !willFollow;
              _followersCountLocal += willFollow ? -1 : 1;
        ...
        return SafeArea(
          child: Column(
            ...
            children: [
                ...
                onTap: () async {
                  ...
                  final confirmed = await showDialog<bool>(
                    ...
                    builder: (context) => AlertDialog(
                      backgroundColor: AppColors.darkSurface,
                      title: Text('Block User', style: GoogleFonts.outfit(color: Colors.white)),
                      content: Text('Are you sure you want to block @${widget.profile.username}? They will not be able to message you, view your profile, or see your posts.', style: GoogleFonts.outfit(color: Colors.white70)),
                      actions: [
                        TextButton(
                        ...
                        TextButton(
                          ...
                          child: Text('Block', style: GoogleFonts.outfit(color: Colors.redAccent, fontWeight: FontWeight.bold)),
                        ),
                      ],
                    ),
                  );
                  if (confirmed == true) {
                    ...
                    if (mounted) {
                      Navigator.pop(context); // Go back since they are now blocked
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Blocked @${widget.profile.username}')),
                      );
                    }
                  }
                },
              ...
              ListTile(
                leading: Icon(isMuted ? Icons.volume_up_rounded : Icons.volume_off_rounded, color: Colors.white),
                title: Text(
                  isMuted ? 'Unmute @${widget.profile.username}' : 'Mute @${widget.profile.username}',
                ...
                onTap: () async {
                  ...
                  if (isMuted) {
                    ...
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Unmuted @${widget.profile.username}')),
                      );
                    }
                  } else {
                    ...
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text('Muted @${widget.profile.username}')),
                      );
                    }
                  }
                },
                          ...
                          child: Hero(
                            ...
                            child: CircleAvatar(
                              ...
                              backgroundImage: widget.profile.photoUrl.isNotEmpty
                                  ? CachedNetworkImageProvider(widget.profile.photoUrl)
                                  : null,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                    ...
                    child: Row(
                      ...
                      children: [
                        Consumer(builder: (context, innerRef, _) {
                          final postsAsync = innerRef.watch(userPostsProvider(widget.profile.uid));
                          return _stat(
                            postsAsync.when(
                        ...
                        Consumer(builder: (context, innerRef, _) {
                          ...
                          final profileStream = innerRef.watch(_otherProfileProvider(widget.profile.uid));
                          final currentCount = profileStream.value?.followersCount ?? _followersCountLocal;
                          ...
                          return InkWell(
                            ...
                            child: _stat('$currentCount', context.tr('profile_followers')),
                          );
                        }),
                        InkWell(
                          onTap: () => Navigator.push(
                          ...
                          child: _stat(widget.profile.followingCount.toString(), context.tr('profile_following')),
                        ),
                        _stat(widget.profile.karma.toString(), context.tr('profile_karma')),
                      ],
                    ),
                  ),
                  ...
                  Text(
                    widget.profile.bio.isNotEmpty ? widget.profile.bio : context.tr('profile_bio_default'),
                    style: GoogleFonts.outfit(
                  ...
                  if (isMe)
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CreatorAnalyticsScreen())),
                        ...
                        Expanded(
                          ...
                          child: SizedBox(
                            ...
                            child: Consumer(builder: (context, innerRef, _) {
                                final followAsync = innerRef.watch(isFollowingProvider(
                                  (myUid: widget.myUid, targetUid: widget.profile.uid)));
                                
                                // Sync local follow state with stream
                                final isFollowing = followAsync.value ?? _isFollowingLocal;
                                
                                return ElevatedButton(
                                  onPressed: _toggleFollow,
                                  style: ElevatedButton.styleFrom(
                                    backgroundColor: isFollowing ? AppColors.darkSurface : AppColors.electricBlue,
                                    foregroundColor: Colors.white,
                                    elevation: 0,
                                  ...
                                  child: Text(
                                    isFollowing ? context.tr('profile_following_status') : context.tr('profile_follow'),
                            ...
                            child: ElevatedButton(
                              onPressed: _isMessageLoading ? null : () async {
                                ...
                                if (safeMyUid == null) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(content: Text(context.tr('login_to_message'))),
                                  );
                                  return;
                                }

                                try {
                                  final chatId = await ref.read(firestoreRepositoryProvider).getOrCreateDirectChat(
                                    safeMyUid,
                                    widget.profile.uid,
                                  );
                                    ...
                                    Navigator.push(
                                      ...
                                      MaterialPageRoute(
                                        builder: (_) => ChatConversationScreen(
                                          ...
                                          userAvatar: widget.profile.photoUrl,
                                        ),
                                      ),
                                    );
                                  }
                                  ...
                                  if (mounted) {
                                    setState(() => _isMessageLoading = false);
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(content: Text(context.tr('start_chat_failed', args: [e.toString()]))),
                                    );
                                  }
                                }
                              },
                  ...
                  Row(
                    children: [
                      _TabButton(label: context.tr('profile_tab_posts'), isActive: true),
                      const SizedBox(width: 24),
                      _TabButton(label: context.tr('profile_tab_vybz'), isActive: false),
                      const SizedBox(width: 24),
                      _TabButton(label: context.tr('profile_tab_tagged'), isActive: false),
                    ],
                  ),
                  const SizedBox(height: 16),
                ],
    ...
    return postsAsync.when(
          ...
          return SliverToBoxAdapter(
            child: Center(
              child: Padding(
                ...
                child: Column(
                  children: [
                    Icon(Icons.camera_alt_outlined, size: 48, color: Colors.grey.shade200),
                    const SizedBox(height: 12),
                    Text(
                            ...
                            child: Center(
                              child: Padding(
                                ...
                                child: Text(
                                  ...
                                  textAlign: TextAlign.center,
                                  style: GoogleFonts.outfit(fontSize: 10, color: AppColors.electricBlue),
                                ),
                              ),
                            ),
                          ),
      ...
      error: (_, __) => SliverToBoxAdapter(
        child: Padding(
          ...
          child: Center(
            child: Text(context.tr('profile_load_failed'), style: GoogleFonts.outfit(color: Colors.grey)),
          ),
        ),
      ),
    );
