...
class SensorCalibrationService {
  ...
  SensorState applyCalibration(SensorState raw) => SensorState(
        ...
        stabilizedTilt: Offset(raw.stabilizedTilt.dx - _baselineX,
            raw.stabilizedTilt.dy - _baselineY),
      );
  ...
  Future<void> startCalibration(Stream<SensorState> rawStream) async {
    ...
    if (_calibXSamples.isNotEmpty) {
      _baselineX =
          _calibXSamples.reduce((a, b) => a + b) / _calibXSamples.length;
      _baselineY =
          _calibYSamples.reduce((a, b) => a + b) / _calibYSamples.length;
          ...
          child: Column(
            ...
            children: [
              Icon(Icons.screen_rotation,
                  size: 80, color: _done ? Colors.green : Colors.blueAccent),
