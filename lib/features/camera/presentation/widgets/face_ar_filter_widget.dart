  ...
  void _startFaceDetection() {
    widget.controller.startImageStream((image) async {
      ...
      try {
        ...
        final imageRotation = InputImageRotationValue.fromRawValue(widget.controller.description.sensorOrientation) ?? InputImageRotation.rotation0deg;
...
class FaceFilterPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double scaleX = size.width / (rotation == InputImageRotation.rotation90deg || rotation == InputImageRotation.rotation270deg ? imageSize.height : imageSize.width);
    final double scaleY = size.height / (rotation == InputImageRotation.rotation90deg || rotation == InputImageRotation.rotation270deg ? imageSize.width : imageSize.height);
      ...
      switch (filterType) {
        ...
        case ARFilterType.animeEyes:
          if (leftEye != null && rightEye != null) {
             _drawAnimeEyes(canvas, leftEye.position, rightEye.position, translateX, translateY, boundingBox.width);
          }
          break;
        case ARFilterType.dogFace:
          if (noseBase != null) {
            _drawDogFace(canvas, noseBase.position, boundingBox, translateX, translateY);
          }
          break;
        ...
        case ARFilterType.sunglasses:
          if (leftEye != null && rightEye != null) {
             _drawSunglasses(canvas, leftEye.position, rightEye.position, translateX, translateY);
          }
          break;
  ...
  void _drawNeonFacePaint(Canvas canvas, FaceContour? contour, Function trX, Function trY) {
  ...
  void _drawAnimeEyes(Canvas canvas, Point<int> left, Point<int> right, Function trX, Function trY, double faceWidth) {
    final radius = faceWidth * 0.15; // Enlarged eyes
    final paint = Paint()
      ..shader = const RadialGradient(
        colors: [Colors.white, Colors.deepPurple, Colors.black],
        stops: [0.1, 0.6, 1.0],
      ).createShader(Rect.fromCircle(center: Offset(trX(left.x), trY(left.y)), radius: radius));
      
    canvas.drawCircle(Offset(trX(left.x.toDouble()), trY(left.y.toDouble())), radius, paint);
    canvas.drawCircle(Offset(trX(right.x.toDouble()), trY(right.y.toDouble())), radius, paint);
    
    // Sparkles
    final sparkle = Paint()..color = Colors.white;
    canvas.drawCircle(Offset(trX(left.x.toDouble()) + radius*0.3, trY(left.y.toDouble()) - radius*0.3), radius*0.2, sparkle);
    canvas.drawCircle(Offset(trX(right.x.toDouble()) + radius*0.3, trY(right.y.toDouble()) - radius*0.3), radius*0.2, sparkle);
  }
  ...
  void _drawDogFace(Canvas canvas, Point<int> nose, Rect bounds, Function trX, Function trY) {
    // Nose dog
    canvas.drawOval(
      Rect.fromCenter(center: Offset(trX(nose.x.toDouble()), trY(nose.y.toDouble())), width: 40, height: 30),
  ...
  void _drawFlowerCrown(Canvas canvas, Rect bounds) {
    ...
    final colors = [Colors.pink, Colors.yellow, Colors.cyan, Colors.purple, Colors.red];
    for (int i = 0; i < 5; i++) {
      canvas.drawCircle(Offset(bounds.left + (i * spacing), y), 15, Paint()..color = colors[i]);
    }
  }
  ...
  void _drawDevilHorns(Canvas canvas, Rect bounds) {
    ...
    final pLeft = Path()..moveTo(leftX-15, y)..lineTo(leftX+15, y)..lineTo(leftX, y-50)..close();
    final pRight = Path()..moveTo(rightX-15, y)..lineTo(rightX+15, y)..lineTo(rightX, y-50)..close();
    
    canvas.drawPath(pLeft, Paint()..color = Colors.redAccent);
    canvas.drawPath(pRight, Paint()..color = Colors.redAccent);
  }

  void _drawSunglasses(Canvas canvas, Point<int> lEye, Point<int> rEye, Function trX, Function trY) {
    ...
    canvas.drawRect(Rect.fromCenter(center: leftCenter, width: dist, height: dist * 0.6), paint);
    canvas.drawRect(Rect.fromCenter(center: rightCenter, width: dist, height: dist * 0.6), paint);
    canvas.drawLine(leftCenter, rightCenter, Paint()..color=Colors.black..strokeWidth=4);
  }

  void _drawGlitchFace(Canvas canvas, Rect bounds) {
    // Simulate split channel by drawing random lines across the face area
    final random = Random();
    final paint = Paint()..color = Colors.cyanAccent.withOpacity(0.5)..strokeWidth = 2;
    final paint2 = Paint()..color = Colors.redAccent.withOpacity(0.5)..strokeWidth = 2;
    
    for (int i=0; i<10; i++) {
       final y = bounds.top + random.nextDouble() * bounds.height;
       canvas.drawLine(Offset(bounds.left - 20, y), Offset(bounds.right + 20, y), (i%2==0) ? paint : paint2);
    }
  }
  ...
  void _drawBeautySmooth(Canvas canvas, Rect bounds) {
      ...
      canvas.drawOval(bounds, Paint()..color = Colors.white.withOpacity(0.1)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 20));
  }
  ...
  void _drawAgeFilter(Canvas canvas, Rect bounds) {
      // Add a slight darkened/grey texture over the face area
       ...
       canvas.drawLine(Offset(bounds.left+10, bounds.top+30), Offset(bounds.right-10, bounds.top+30), Paint()..color=Colors.black26..strokeWidth=2);
  }
