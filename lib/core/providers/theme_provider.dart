...
final themeProvider =
    StateNotifierProvider<ThemeNotifier, GotchaaThemeState>(ThemeNotifier.new);
...
class ThemeNotifier extends StateNotifier<GotchaaThemeState> {
  ...
  void _loadFromPrefs() {
    ...
    final darkTypeName =
        prefs.getString('dark_theme_type') ?? ThemeType.midnightPurple.name;
    final lightTypeName =
        prefs.getString('light_theme_type') ?? ThemeType.softLavender.name;
    final cornerRadius = prefs.getDouble('message_corner_radius') ?? 16.0;
    final chatLines = prefs.getInt('chat_list_view_lines') ?? 2;

    final darkType = ThemeType.values.firstWhere((e) => e.name == darkTypeName,
        orElse: () => ThemeType.midnightPurple);
    final lightType = ThemeType.values.firstWhere(
        (e) => e.name == lightTypeName,
        orElse: () => ThemeType.softLavender);
  ...
  void _updateCurrentTheme() {
    ...
    state = state.copyWith(
        currentTheme: theme.copyWith(cornerRadius: state.messageCornerRadius));
  }
