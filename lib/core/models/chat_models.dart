...
class ChatModel {
  ...
  ChatModel({
    ...
    required this.unreadCount, required this.typing, required this.isArchived, required this.isMuted, this.lastMessage = '',
  ...
  factory ChatModel.fromMap(Map<String, dynamic> data, String id) {
    ...
    final participantsRaw = data['participants'] ?? data['participantIds'] ?? data['members'];
    final parsedParticipants = participantsRaw is List
        ? participantsRaw.map((e) => e.toString()).where((e) => e.isNotEmpty).toList()
        : <String>[];

    final namesRaw = data['participantNames'] ?? data['participantNameMap'] ?? data['participantInfo'];
    final avatarsRaw = data['participantAvatars'] ?? data['participantAvatarMap'];
    ...
    return ChatModel(
      ...
      participantAvatars: avatarsRaw is Map
          ? avatarsRaw.map((k, v) => MapEntry(k.toString(), v?.toString() ?? ''))
          : {},
      lastMessage: lastMsg,
      ...
      unreadCount: data['unreadCount'] is Map 
          ? (data['unreadCount'] as Map).map((k, v) => MapEntry(k.toString(), v is int ? v : int.tryParse(v.toString()) ?? 0))
          : {},
      typing: data['typing'] is Map 
          ? (data['typing'] as Map).map((k, v) => MapEntry(k.toString(), v == true))
          : {},
      isArchived: data['isArchived'] is Map 
          ? (data['isArchived'] as Map).map((k, v) => MapEntry(k.toString(), v == true))
          : {},
      isMuted: data['isMuted'] is Map 
          ? (data['isMuted'] as Map).map((k, v) => MapEntry(k.toString(), v == true))
          : {},
    );
  }
  final String id;
...
class ReplyTo {
  ...
  ReplyTo({required this.messageId, required this.senderId, required this.text, required this.type});
...
class MessageFactory {
  ...
  static Map<String, dynamic> createMessageMap({
    ...
    if (text.isEmpty && type == 'text') throw ArgumentError('text cannot be empty for text messages');
    if (!['text', 'image', 'audio', 'video', 'deleted', 'system'].contains(type)) {
...
class MessageModel {
  ...
  factory MessageModel.fromMap(Map<String, dynamic> data, String id) {
    final dynamic textRaw = data['text'] ?? data['message'] ?? data['body'] ?? data['content'];
    String messageText = '';
    ...
    final dynamic senderRaw = data['senderId'] ?? data['senderID'] ?? data['sender'] ?? data['from'];
    final dynamic receiverRaw = data['receiverId'] ?? data['receiverID'] ?? data['receiver'] ?? data['to'];
    final dynamic timestampRaw = data['timestamp'] ?? data['createdAt'] ?? data['time'];
    final dynamic encryptedRaw = data['isEncrypted'] ?? data['encrypted'] ?? data['is_encrypted'];
    final bool parsedIsEncrypted = encryptedRaw == true || _looksLikeEncryptedPayload(messageText);

    return MessageModel(
      id: id,
      schemaVersion: data['schemaVersion'] is int ? data['schemaVersion'] : 0, // 0 for legacy
      senderId: senderRaw?.toString() ?? '',
      receiverId: receiverRaw?.toString() ?? '',
      text: messageText,
      type: (data['type']?.toString().isNotEmpty ?? false) ? data['type'].toString() : 'text',
      mediaUrl: data['mediaUrl']?.toString(),
      mediaThumbnailUrl: data['mediaThumbnailUrl']?.toString(),
      audioDuration: data['audioDuration'] is int ? data['audioDuration'] : null,
      replyTo: data['replyTo'] != null ? ReplyTo.fromMap(data['replyTo']) : null,
      ...
      isDeletedForEveryone: data['isDeletedForEveryone'] == true || data['type'] == 'deleted',
      isDeletedFor: data['isDeletedFor'] is List
          ? List<String>.from(data['isDeletedFor'])
          : (data['deletedFor'] is List ? List<String>.from(data['deletedFor']) : []),
