...
void main() {
  ...
  group('AnalyticsService Tests', () {
    test('Every event method is callable and does not throw', () async {
      ...
      await AnalyticsService.logMatchFound(
          matchedLanguage: 'en', waitTimeSeconds: 5);
      await AnalyticsService.logTipSent(amount: 10, recipientUid: 'user_123');
      await AnalyticsService.logLanguageSwitched(
      ...
      await AnalyticsService.logContentLiked(
          contentType: 'post', isLiked: true);
    ...
    test('All events tested individually', () async {
      ...
      expect(() => AnalyticsService.logSignUpComplete(method: 'email'),
          returnsNormally);
      expect(() => AnalyticsService.logLoginSuccess(method: 'email'),
          returnsNormally);
      expect(() => AnalyticsService.logOnboardingComplete(language: 'en'),
          returnsNormally);
      expect(() => AnalyticsService.logFirstPostCreated(postType: 'text'),
          returnsNormally);
      expect(() => AnalyticsService.logPostCreated(postType: 'text'),
          returnsNormally);
      expect(() => AnalyticsService.logFirstMessageSent(), returnsNormally);
      expect(
          () => AnalyticsService.logMessageSent(type: 'text'), returnsNormally);
      expect(() => AnalyticsService.logVybzCreated(durationSeconds: 10),
          returnsNormally);
      expect(
      ...
      expect(
          () =>
              AnalyticsService.logTipSent(amount: 10, recipientUid: 'user_123'),
          returnsNormally);
      expect(
      ...
      expect(() => AnalyticsService.logFeatureViewed(featureName: 'vibetalk'),
          returnsNormally);
      expect(
